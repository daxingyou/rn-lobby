...
const Layout = layout[7]
const objData = Layout
const groupTitle = Layout.totleTitle

class LianXiaoPage extends Component {
  ...
  _onPressCancelInput() {
    this.setState({ btnStateArrKeeping: objData, money: "", sendAction: {gameNum: 8}, keepingCount: 0 })
  }
  _onPressRecentLottery() {
    this.setState({ isRecentLotteryVisible: !this.state.isRecentLotteryVisible })
  }
  _onPressConfirm() {
    const typeArr = ["LX2", "LX3", "LX4", "LX5"]
    ...
    switch(typeArr[this.state.activeTab]){
      case "LX5":
            ...
            <ScrollableTabView page={this.state.activeTab} renderTabBar={() => <SelectedTab textArr={groupTitle} />}
                               onChangeTab={(tab)=>{ this._onPressCancelInput();this.setState({activeTab: tab.i}, () => {this.props.onChangeTab && this.props.onChangeTab(tab.i)}) }}>
              {
                this.state.btnStateArrKeeping.items.map((item, index) => {
                  return (
                    <ScrollView style={{borderTopWidth: 2, borderTopColor: '#fff'}} key={index} automaticallyAdjustContentInsets={false}
horizontal={false}>
...
export default connect(mapStateToProps, mapDispatchToProps)(LianXiaoPage)
