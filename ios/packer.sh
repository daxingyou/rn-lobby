...
#	cp $constant_file ${constant_file}.bp
	...
	echo "sync match"
	fastlane match --git_branch ${apple_id} -y appstore -a ${app_bundle_id} -u ${apple_id} -r "https://gitlab.kosun.net/iOS-Team/Certificates.git" --force_for_new_devices
...
#	sed -i '' "s/${YOUR_APP_NAME}/${app_name}/" $constant_file
	...
	ipa_path='Build/'
	ipa_name=${app_product_name}_${app_version_string}_${app_bundle_id}
	echo ">> run gym"
